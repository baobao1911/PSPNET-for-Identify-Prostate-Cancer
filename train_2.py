...
from Model.Model2 import MyModel
...
def model_training(train_img_path, train_mask_path,
    ...
    model = MyModel(n_classes=n_classes, zoom_factor=16, loss=loss_fn).to(device)
    modules_ori = [model.block, model.block0, model.block1, model.block2_relu, model.block2, model.block3, model.block4, model.block5,  model.block6,  
    ...
    modules_new = [model.ppm, model.conv_ppm, model.gau1, model.gau2, model.aux, model.final_conv]
    ...
    for epoch in range(ep, epochs):
        ...
        if epoch == 1 or (val_loss[-1] <= min(val_loss[:-1])):
            ...
            file_path = r'D:/University/MyProject/Source/CheckPoints/train2_5/best_model.pth'
            if os.path.exists(file_path):
                os.remove(file_path)  # You can also use os.unlink(file_path)
            print(f'Update best model file')
            torch.save(checkpoint, f'D:/University/MyProject/Source/CheckPoints/train2_5/best_model.pth')
...
if __name__ == "__main__":
    ...
    result_path = r'D:\University\MyProject\Source\data_result_training\train2_5.csv'
    batch_s = 10
    n_workers = 6
    n_classes = 6
    base_lr = 0.045
