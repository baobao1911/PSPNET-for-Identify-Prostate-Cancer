...
def norm_HnE(image, Io=240, alpha=1, beta=0.15):
    ...
    # OD = −log10(I)  
    #OD = -np.log10(image+0.004)  #Use this when reading images with skimage
    #Adding 0.004 just to avoid log of zero. 

    OD = -np.log10((image.astype(np.float32)+1)/Io) #Use this for opencv imread
...
if __name__ == "__main__":
    path_img_train = f'D:/University/MyProject/Data/gleasoncropped/train/images'
    path_img_valid = f'D:/University/MyProject/Data/gleasoncropped/test/images'

    train_img_list = os.listdir(path_img_train)
    valid_img_list = os.listdir(path_img_valid)

    for file in train_img_list:
        image = cv2.imread(f'{path_img_train}/{file}', 1)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = np.array(image, dtype=np.float32)

        if image.mean() < 230 and image.std() > 15:
            print(f'{file}')
            image_norm, H, E = norm_HnE(image)
            if not os.path.exists(f'{path_img_train}_norm'):
                os.makedirs(f'{path_img_train}_norm')
            plt.imsave(f'{path_img_train}_norm/{file}', image_norm)
    print('train success')

    for file in valid_img_list:
        image = cv2.imread(f'{path_img_valid}/{file}', 1)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = np.array(image, dtype=np.float32)
        ...
        if image.mean() < 230 and image.std() > 15:
            print(f'{file}')
            image_norm, H, E = norm_HnE(image)
            if not os.path.exists(f'{path_img_valid}_norm'):
                os.makedirs(f'{path_img_valid}_norm')
            plt.imsave(f'{path_img_valid}_norm/{file}', image_norm)
    print('valid success')