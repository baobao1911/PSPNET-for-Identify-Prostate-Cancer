...
from Model.PSP_HDC_DUC import PSP_HDC_DUC
...
def model_training(train_img_path, train_mask_path,
    ...
    model = PSP_HDC_DUC(classes=n_classes, zoom_factor=8, criterion=loss_fn).to(device)
    ...
    for epoch in range(ep, epochs):
        ...
        if epoch == 1 or (val_loss[-1] <= min(val_loss[:-1])):
            ...
            file_path = r'D:\University\Semantic_Segmentation_for_Prostate_Cancer_Detection\Semantic_Segmentation_for_Prostate_Cancer_Detection\Training_result\ModelSave\PSPNet_HDC_DUC.pth'
...
if __name__ == "__main__":
    ...
    result_path = r'D:\University\Semantic_Segmentation_for_Prostate_Cancer_Detection\Semantic_Segmentation_for_Prostate_Cancer_Detection\Training_result\Result_info\PSPNet_HDC_DUC.csv'
    batch_s = 4
    n_workers = 6
    n_classes = 6
    base_lr = 0.01
